...
var dugmePokusajPonovo = "<button id='pokusajPonovo' class='btn btn-warning btn-block' onclick='pokusajPonovo()' >Pokušaj ponovo</button>";

function pocniIgru() {
// Odaberi random pitanje
...
// Odaberi random odgovor na njega
}
...
function provjeriPobjedu() {
    if(trenutnaRijec == trazenaRijec) {
        document.getElementById("poruka").innerHTML="<div class='alert alert-success text-center'>Cestitamo</div>";
...
function provjeriPoraz() {
    if(dozvoljenoPokusaja == brojacPokusaja) {
        document.getElementById("poruka").innerHTML="<div class='alert alert-success text-center'>Trazena rijec je:"+trazenaRijec+"</div>";
        document.getElementById("pokusajPonovo").innerHTML=dugmePokusajPonovo;
        zabraniDugmad();
    }
}
